...
```json
{
  ...
  "onboarding_files": [
    {
      "path": "requests/bugfix-requests/verify-batching-guard-red-on-arrival/ROOT_CAUSE_ANALYSIS.md",
    ...
    {
      "path": "requests/bugfix-requests/verify-batching-guard-red-on-arrival/BUGFIX_REQUEST.md",
    ...
    {
      "path": "requests/bugfix-requests/doc-link-guard-mismatch/ROOT_CAUSE_ANALYSIS.md",
  ...
  "files_to_touch": [
    ...
    {
      "path": "requests/bugfix-requests/verify-batching-guard-red-on-arrival/IMPLEMENTATION_PLAN.md",
    ...
    },
    {
      ...
      "change": "DO NOT EDIT. Its contract is the gated decision owned by `requests/bugfix-requests/doc-link-guard-mismatch/`. Read `:10-33` to state its behaviour accurately in Phase 3."
    }
  ],
  "code_references": [
    {
    ...
    {
      "ref": "requests/bugfix-requests/doc-link-guard-mismatch/ROOT_CAUSE_ANALYSIS.md:95-98",
...
```json
{
  ...
  "phases": [
    ...
    },
    {
      ...
      "steps": [
        ...
        "DO NOT TOUCH THE PROSE AROUND THOSE REFERENCES. The 'What good looks like' bullets that promise fenced-content exemption, `file.py:123` suffixes, `var/` targets, and a bare-`requests/...`-token scan (see `.claude/skills/create-implementation-plan/SKILL.md:250-256` as the canonical instance) are the subject of the GATED decision in `requests/bugfix-requests/doc-link-guard-mismatch/ROOT_CAUSE_ANALYSIS.md:79-93`. Editing them here decides that call by implication, which both RCAs name as the worst available outcome. Change the reference token only.",
    ...
    },
    {
      ...
      "steps": [
        ...
        "Do NOT extend `tests/test_skill_references.py` from two token classes to every repo path a skill names. That is Hardening item 6 here and is explicitly GATED in `requests/bugfix-requests/doc-link-guard-mismatch/ROOT_CAUSE_ANALYSIS.md:104-106`; it belongs to that request's plan.",
  ...
  "code_references": [
    ...
    {
      "ref": "requests/bugfix-requests/doc-link-guard-mismatch/ROOT_CAUSE_ANALYSIS.md:79-93",
...
```json
{
  ...
  "phases": [
    ...
    },
    {
      ...
      "steps": [
        ...
        "Change the file name and NOTHING ELSE on those lines. Do not touch the surrounding sentences that promise fence exemption, a `file.py:123` suffix, `var/` targets, or a bare-token scan. Those promises are the subject of `requests/bugfix-requests/doc-link-guard-mismatch/`'s gated decision and are not this request's to settle. Renaming the guard TO `test_request_links.py` is the one option that is off the table: it scans all Markdown, not just `requests/`, so the ported name misdescribes it (that request's RCA, its Fix-posture blockquote).",
    ...
    },
    {
      ...
      "steps": [
        ...
        "State plainly what this request does NOT close: `requests/bugfix-requests/doc-link-guard-mismatch/`'s gated decision is still open, so the code-span citation convention remains mandatory for every artifact; and `requests/bugfix-requests/leak-guard-blind-to-untracked-files/` is untouched.",
  ...
  "files_to_touch": [
    ...
    {
      "path": "requests/bugfix-requests/verify-batching-guard-red-on-arrival/IMPLEMENTATION_REPORT.md",
