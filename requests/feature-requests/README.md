...
| [agent-memory-curation](agent-memory-curation/) | intake | The `data-engineer`'s memory file hit its 250-line CI ceiling while its own rules say *append freely, never prune* — no legal move at the boundary, mid-build. Removes the ceiling and moves curation to `/update-docs`, where the judgment and the authority already live. **Direct build**, argued: subtraction plus prose across five process artifacts. The genus split (method vs tooling) is explicitly deferred until a sweep produces the evidence |
