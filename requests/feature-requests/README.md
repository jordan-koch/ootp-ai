...
| Feature | Stage | Notes |
|---|---|---|
| [first-sight](first-sight/) | planned | The `.dat` parser, warehouse landing, a data catalog, and two starting reports — the first time the GM can see its own club. Scoped as **reshape**: ratings decoupled behind the scouted-view spike, standings ship structural-only, dbt deferred. Planned in 14 phases; `tests/` is main-thread-authored because it is in the builder's deny set |
...
| [open-front-office](open-front-office/) | intake | Rules refactor: the experiment is infrastructure + adaptive agent, not simulated attention scarcity — so the report wall and action economy go, replaced by a granted `gm_view` schema (ADR 0012 enforced by grant, not prose), an execution log, and append-only GM memory (journal, predictions, trigger queue, mutable plan). Supersedes 0013/0016/0018/0019, amends 0014/0017; pins first-sight Phases 10–11, gm-inbox, and news-subscription-dial |
