...
**What Challenge-mode saves *can* be validated against**, given no export: Tier A (`players.csv` joined on the embedded Lahman ID) proves identity and names for real players on Boston directly; residual-free byte accounting proves the walk consumed what it should; and the cross-mode equivalence above proves the format did not shift. A strong triangle — but evidence by construction, not a row-for-row diff.

> **Amendments applied after the first commit of this plan** (`acc5575`), each at the operator's direction: `ingest_seq` added to every bronze key and `snapshot_date` renamed to `sim_date` (§2.3(d)); this §2.5 and the cross-mode equivalence test; a separate `ootp_dev` landing schema for development (Phase 1); and a written spawn contract for handing the GM its reports (Phase 11). A world-state or "situation" artifact was **considered and deliberately rejected** — it would be a third report, which the scope rules out as a non-goal, and the thin-sight tension is the thing the experiment is testing.
...
**Acceptance.** AC9's in-phase clauses green (the display-name clause belongs to Phase 7); AC12 green at both tiers with the residual recorded; **`tests/test_cross_mode_format.py` extended to `players.dat`** — same record shape and stride across both test saves (§2.5); the `list_id` label at `inferred` or better with evidence written down, or the opaque-integer fallback in force; the population claim measured; `test_read_only.py` re-run green after the largest read this project performs.
