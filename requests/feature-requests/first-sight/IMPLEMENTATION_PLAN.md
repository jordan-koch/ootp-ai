...
**Steps.**
1. Complete `contracts/tables.toml` and `field_map.toml` per §2.3(b). Declared keys: `bronze_team` (`snapshot_date`, `save_id`, `team_id`); `bronze_player` (`snapshot_date`, `save_id`, `player_id`); `bronze_team_roster` (`snapshot_date`, `save_id`, `team_id`, `player_id`, `list_id`) — **explicitly not** `(snapshot_date, player_id)`; `bronze_name` (`snapshot_date`, `save_id`, `name_space`, `name_index`) with its own declared grain, key and coverage like every other table. Record Decisions §8 here too (ratings render at the 20–80 player-page scale) so the next slice inherits a decision rather than re-deriving one.
...
**Steps.**
1. `reports/__main__.py` exposing `render`, so that `uv run python -m ootp_ai.reports render` is the real entry point AC14 invokes. Output to `<output_root>/<save_id>/<snapshot_date>/` per §2.4.
...
| P11 | **`bronze_name` is keyed `(snapshot_date, save_id, name_space, name_index)`** | It is genuinely unknown whether `names.dat` carries one index space or two. That key is correct under **both** outcomes and costs one column; the alternative silently collides every row if the answer is two. |
| P12 | **`ingest_run` is keyed `(snapshot_date, save_id)` and a re-land refuses loudly** | Resolves a real collision: AC10 demands byte-identity across a re-load, but an append-only ingest table adds a row and a wall-clock column breaks it. This satisfies all four AC10 clauses at once. An implementer who misses this writes a test that cannot pass. |
...
lifetime. The scope decided `snapshot_date` goes in every primary key, so this plan honours
...
| Table | Grain | Key |
|---|---|---|
| `bronze_team` | one row per team per save per snapshot | `snapshot_date, save_id, team_id` |
| `bronze_player` | one row per player per save per snapshot | `snapshot_date, save_id, player_id` |
| `bronze_team_roster` | one row per player per team per **roster list** per save per snapshot | `snapshot_date, save_id, team_id, player_id, list_id` |
| `bronze_name` | one row per name-table entry per save per snapshot | `snapshot_date, save_id, name_space, name_index` |
...
**Update semantics.** Append-only per snapshot; snapshots immutable; a re-land of an
existing `(snapshot_date, save_id)` refuses loudly.
