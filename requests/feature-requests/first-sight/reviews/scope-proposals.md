...
- tests/test_doc_links.py fails CI on any tracked Markdown link into var/ (a live defect, requests/bugfix-requests/doc-link-guard-mismatch symptom B, which names `var/snapshots/2024-03-07/` as a reproduction). Pointing the GM's forced-read list at a var/ path via a tracked link turns CI red, and on a fresh clone the target does not exist at all. The GM must be handed report paths at spawn time by the umpires.
